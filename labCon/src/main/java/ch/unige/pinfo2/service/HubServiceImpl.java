...
/**
 * Provides a set of service for the hub of the smartlab.
 */
public class HubServiceImpl implements HubService {
	...
	private ArrayList<Socket> fuseSocketStates(ArrayList<Socket> socketStates1, ArrayList<Socket> socketStates2) {
		...
		while (true) {
			// if a list is empty, append the remaining elements of the other
			// list
			if (socketStates1.isEmpty()) {
				while (!socketStates2.isEmpty()) {
					fusedSocketStates.add(new Socket("fusedSocket", socketStates2.get(0).getTimestamp(),
							socketStates2.get(0).getCurrent() + lastCurrent1,
							socketStates2.get(0).getPower() + lastPower1));
					socketStates2.remove(0);
				}
				return fusedSocketStates;
			}
			if (socketStates2.isEmpty()) {
				while (!socketStates1.isEmpty()) {
					fusedSocketStates.add(new Socket("fusedSocket", socketStates1.get(0).getTimestamp(),
							socketStates1.get(0).getCurrent() + lastCurrent2,
							socketStates1.get(0).getPower() + lastPower2));
					socketStates1.remove(0);
				}
				return fusedSocketStates;
			}

			// find the first element of the two lists and add it taking into
			// account the last known state of the other socket at a time prior
			// or equal
			if (socketStates1.get(0).getTimestamp().longValue() < socketStates2.get(0).getTimestamp().longValue()) {
				fusedSocketStates.add(new Socket("fusedSocket", socketStates1.get(0).getTimestamp(),
						socketStates1.get(0).getCurrent() + lastCurrent2,
				...
				lastPower1 = socketStates1.get(0).getPower().doubleValue();
				socketStates1.remove(0);
			} else if (socketStates1.get(0).getTimestamp().longValue() > socketStates2.get(0).getTimestamp()
					.longValue()) {
				fusedSocketStates.add(new Socket("fusedSocket", socketStates2.get(0).getTimestamp(),
						socketStates2.get(0).getCurrent() + lastCurrent1,
						socketStates2.get(0).getPower() + lastPower1));
				...
				fusedSocketStates.add(new Socket("fusedSocket", socketStates1.get(0).getTimestamp(),
						socketStates1.get(0).getCurrent() + socketStates2.get(0).getCurrent(),
						socketStates1.get(0).getPower() + socketStates2.get(0).getPower()));
				lastCurrent1 = socketStates1.get(0).getCurrent().doubleValue();
				lastPower1 = socketStates1.get(0).getPower().doubleValue();
				lastCurrent2 = socketStates2.get(0).getCurrent().doubleValue();
				lastPower2 = socketStates2.get(0).getPower().doubleValue();
	...
	public List<Hub> getStates(String deviceId, Long from, Long to) {
		...
		for (Socket fusedSocketState : fusedSocketStates) {
			hubStates.add(new Hub(deviceId, fusedSocketState.getTimestamp(), fusedSocketState.getCurrent(),
					fusedSocketState.getPower()));
		}
